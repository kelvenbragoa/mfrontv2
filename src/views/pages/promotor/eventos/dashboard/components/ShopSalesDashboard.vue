...
import { useShopSalesDashboard } from '@/composables/useShopSalesDashboard';
...
const confirmCancel = (row) => {
    ...
    confirm.require({
        header: 'Cancelar encomenda',
        message: 'A encomenda será marcada como cancelada e o stock será reposto. O pagamento M-Pesa não é revertido automaticamente.',
        icon: 'pi pi-exclamation-triangle',
        rejectLabel: 'Voltar',
        acceptLabel: 'Cancelar encomenda',
        acceptClass: 'p-button-danger',
        accept: () => cancelOrder(row)
    });
};
    ...
    <div class="sales-dashboard">
        <div class="flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
            <div>
                ...
                <h4 class="m-0 text-900">Dashboard da loja</h4>
                <span class="text-600">Vendas, levantamento e cancelamentos da loja do evento.</span>
            </div>
            <div class="flex flex-wrap gap-2">
                <Button icon="pi pi-refresh" label="Atualizar" outlined :loading="isRefreshing" @click="getData({ silent: true })" />
                <Button icon="pi pi-file-pdf" label="PDF" outlined :loading="loadingPdf" @click="downloadPdf" />
            </div>
        </div>

        <div v-if="isLoading" class="grid">
            <div v-for="n in 6" :key="`sk-${n}`" class="col-12 md:col-6 xl:col-4">
        ...
        <template v-else>
            <div class="grid">
                <div v-for="kpi in kpis" :key="kpi.key" class="col-12 md:col-6 xl:col-4">
            ...
            <div class="card mt-3">
                <div class="filter-bar mb-3">
                    <IconField iconPosition="left" class="filter-bar__search">
                        ...
                        <InputText v-model="searchQuery" placeholder="Procurar encomendas..." class="w-full" />
                    </IconField>
                    <Dropdown
                ...
                <p v-else class="tab-empty">Sem produtos vendidos para filtrar.</p>
            </div>
            ...
            <div class="card mt-3">
                <div class="flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                    <h5 class="m-0">Encomendas da loja</h5>
                ...
                >
                    <Column header="Produtos">
                        <template #body="slotProps">{{ slotProps.data.product_names || '--' }}</template>
                    </Column>
                    ...
                    <Column header="Levantamento">
                        <template #body="slotProps">
                            <Tag :value="orderStatus(slotProps.data).label" :severity="orderStatus(slotProps.data).severity" />
                        </template>
                    </Column>
                    <Column header="" style="width: 8rem">
                        <template #body="slotProps">
                            <Button
                                v-if="slotProps.data.can_cancel"
                                label="Cancelar"
                                icon="pi pi-times"
                                size="small"
                                text
                                severity="danger"
                                :loading="cancellingId === slotProps.data.id"
                ...
                <p v-else class="tab-empty">
                    {{ hasActiveFilters ? 'Nenhuma encomenda corresponde aos filtros.' : 'Ainda não há encomendas registadas.' }}
                </p>
            </div>
        </template>
    </div>

...
const { defineField, handleSubmit, errors, setErrors, setValues, setFieldValue } = useForm({
...
const goBack = () => router.push(`/promotor/eventos/${eventId}`);
...
const clearNewImage = () => {
    ...
    imagePreview.value = existingImage.value ? storageURL + existingImage.value : null;
};

const displayedImage = computed(() => {
    if (imagePreview.value) return imagePreview.value;
    if (existingImage.value && !imageBroken.value) return storageURL + existingImage.value;
    return null;
});
...
const onSubmit = handleSubmit(async (values) => {
    ...
    try {
        await axios.post(`${baseURL}/promotor-eventos/${eventId}`, payload, {
        ...
        toast.add({
            ...
            detail: 'As alterações foram guardadas.',
            life: 3000
        });

        router.push(`/promotor/eventos/${eventId}`);
    } catch (error) {
        toast.add({
            severity: 'error',
...
const loadForm = async () => {
    ...
    try {
        const response = await axios.get(`${baseURL}/promotor-eventos/${eventId}/edit`);
        ...
        if (status === 404) {
            ...
            loadError.value = 'Não tens permissão para editar este evento.';
        } else if (status === 401) {
            loadError.value = 'A sessão expirou. Inicia sessão novamente.';
        } else {
            loadError.value = 'Não foi possível carregar o formulário.';
        }
    } finally {
    ...
    <div class="event-form-page">
        <div class="flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
            <div>
                ...
                <h4 class="m-0 text-900">Editar evento</h4>
                <span class="text-600">Atualiza os dados do teu evento.</span>
            </div>
        </div>
        ...
        <div v-else-if="loadError" class="card empty-state">
            ...
            <h5 class="text-900 mb-2">Não foi possível editar</h5>
        ...
        <form v-else class="p-fluid" @submit.prevent="onSubmit">
            <div class="card mb-3">
                ...
                <div class="field">
                    ...
                    <Textarea id="description" v-model="description" rows="5" autoResize :class="{ 'p-invalid': errors.description }" />
            ...
            <div class="card mb-3">
                ...
                <p class="text-600 mt-0 mb-4">Mantém a atual ou escolhe uma nova (até 1 MB).</p>

                <div class="image-row">
                    <div v-if="displayedImage" class="image-preview-wrap">
                        <img :src="displayedImage" alt="Capa do evento" class="image-preview" @error="imageBroken = true" />
                        <Button
                            v-if="imageFile"
                            type="button"
                            icon="pi pi-times"
                    ...
                    <div v-else class="image-placeholder">
                        <i class="pi pi-image text-3xl text-400" />
                    </div>
                    <FileUpload
                        mode="basic"
                        name="image"
            ...
            <div class="flex flex-wrap justify-content-end gap-2">
                ...
                <Button type="submit" label="Guardar alterações" icon="pi pi-check" :loading="isSubmitting" />
            </div>
        </form>
    </div>
</template>

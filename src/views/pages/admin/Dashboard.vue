...
<template>
    <div class="admin-dashboard">
        <div class="flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
            <div>
                <h4 class="m-0 text-900">Painel de administração</h4>
                <span class="text-600">Visão geral dos últimos {{ rangeLabel.toLowerCase() }}</span>
            </div>
        ...
        <template v-else>
                ...
                <div class="col-12 xl:col-4">
                    <div class="card h-full">
                        <div class="flex align-items-center justify-content-between mb-4">
                            ...
                            <router-link to="/admin/eventos" class="text-primary text-sm no-underline">Ver todos</router-link>
                        </div>
                        ...
                        <div class="summary-split mt-4">
                            ...
                            <div>
                                <span class="block text-500 text-sm">Promotores</span>
                                <span class="text-900 font-medium text-xl">{{ formatNumber(totals.promotors) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="grid">
                <div class="col-12 md:col-6 xl:col-3">
                    <router-link to="/admin/transacoes" class="mini-card">
                        <div>
                            <span class="block text-500 text-sm mb-1">Pagamentos por confirmar</span>
                            <span class="text-900 font-medium text-xl">{{ formatNumber(totals.pending_payments) }}</span>
                        </div>
                        <i class="pi pi-clock text-orange-500 text-xl" />
                    </router-link>
                </div>
                <div class="col-12 md:col-6 xl:col-3">
                    <router-link to="/admin/eventos" class="mini-card">
                        <div>
                            <span class="block text-500 text-sm mb-1">Eventos à espera de aprovação</span>
                ...
                <div class="col-12 md:col-6 xl:col-3">
                    <router-link to="/admin/tickets" class="mini-card">
                        <div>
                        ...
                        <i class="pi pi-ticket text-green-500 text-xl" />
                    </router-link>
                </div>
                <div class="col-12 md:col-6 xl:col-3">
            ...
            <div class="grid">
                        ...
                        <ul v-if="dashboard.top_events.length" class="list-none p-0 m-0">
                            <li v-for="item in dashboard.top_events" :key="item.event_id" class="top-event">
                                ...
                                <div class="top-event__info">
                                    <router-link :to="`/admin/eventos/${item.event_id}`" class="top-event__name">
                                        {{ item.name }}
                                    </router-link>
                                    <span class="text-500 text-sm">
                                        {{ item.promotor || 'Promotor desconhecido' }} · {{ formatDate(item.start_date) }}
                                    </span>
                                </div>
                ...
                <div class="col-12 xl:col-5">
                    <div class="card h-full">
                        <div class="flex align-items-center justify-content-between mb-4">
                            <h5 class="m-0">Aguardam aprovação</h5>
                            <router-link to="/admin/eventos" class="text-primary text-sm no-underline">Gerir</router-link>
                        </div>

                        <ul v-if="dashboard.pending_events.length" class="list-none p-0 m-0">
                            <li v-for="item in dashboard.pending_events" :key="item.id" class="pending-event">
                                <div class="pending-event__info">
                                    <router-link :to="`/admin/eventos/${item.id}`" class="pending-event__name">
                                        {{ item.name }}
                                    </router-link>
                                    <span class="text-500 text-sm">
                                        {{ item.promotor || 'Promotor desconhecido' }} · submetido {{ formatDate(item.created_at) }}
                                    </span>
                                </div>
                                <Tag
                                    :severity="statusMeta[item.status_id]?.severity ?? 'info'"
                                    :value="statusMeta[item.status_id]?.label ?? 'Pendente'"
                                />
                            </li>
                        </ul>
                        ...
                        <div v-else class="empty-state">
                            ...
                            <p class="text-600 m-0">Não há eventos por aprovar.</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="flex align-items-center justify-content-between mb-4">
                            ...
                            <router-link to="/admin/tickets" class="text-primary text-sm no-underline">Ver bilhetes</router-link>
                        </div>
                        ...
                        >
                            ...
                            <Column field="event_name" header="Evento">
                                <template #body="slotProps">
                                    <router-link :to="`/admin/eventos/${slotProps.data.event_id}`" class="text-primary no-underline">

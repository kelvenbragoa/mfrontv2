...
const onSubmit = handleSubmit((values) => {
    ...
    axios
        .put(`${baseURL}/promotor-bar/${router.currentRoute.value.params.idbar}`, values)
        .then((response) => {
            resetForm();
            router.back();
            toast.add({ severity: 'success', summary: `Successo`, detail: 'Bilhete alterado com sucesso', life: 3000 });
        })
        .catch((error) => {
...
const getData = () => {
    axios
    .get(`${baseURL}/promotor-bar/${router.currentRoute.value.params.idbar}/edit`)
        .then((response) => {
